...
res = st.session_state.all_data["RELINFRA"]
...
while True:
    ...
    if time.time() - st.session_state.last_analysis_time >= 100:
        ...
        st.session_state.res['CMP'] = round(get_cmp_price(secrets["connections"]["gsheets"]["worksheets"]['RELINFRA']),2)
        ...
        # st.session_state.res = res
        title.title('')
        title.title(f'Data for RELINFRA')
        res_place.text('')
