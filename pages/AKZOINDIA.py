...
res = st.session_state.all_data["AKZOINDIA"]
...
while True:
    ...
    if time.time() - st.session_state.last_analysis_time >= 100:
        ...
        st.session_state.res['CMP'] = round(get_cmp_price(secrets["connections"]["gsheets"]["worksheets"]['AKZOINDIA']),2)
        ...
        # st.session_state.res = res
        title.title('')
        title.title(f'Data for AKZOINDIA')
        res_place.text('')
