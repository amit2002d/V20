...
res = st.session_state.all_data["BAJAJHIND"]
...
while True:
    ...
    if time.time() - st.session_state.last_analysis_time >= 100:
        ...
        st.session_state.res['CMP'] = round(get_cmp_price(secrets["connections"]["gsheets"]["worksheets"]['BAJAJHIND']),2)
        ...
        # st.session_state.res = res
        title.title('')
        title.title(f'Data for BAJAJHIND')
        res_place.text('')
